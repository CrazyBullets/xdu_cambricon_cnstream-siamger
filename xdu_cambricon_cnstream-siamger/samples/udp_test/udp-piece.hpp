...
#define HEAD_POS_TOTAL_PIECES 	6 	// 所有分片的数量
#define HEAD_POS_P_INDEX 		8 	// 分片序号，从0开始
#define HEAD_POS_P_LENGTH 		10 	// 当前分片数据的大小
#define HEAD_SIZE 				12	// 头部大小
// #define PIECE_FIX_SIZE 			10240 	//每个分片数据最大字节数
#define PIECE_FIX_SIZE 			(576UL - 8UL - 20UL - 12UL) 	//每个分片数据最大字节数
...
typedef struct udp_piece
{
    ...
    unsigned char *recv_buf;			// 保存接收数据
    unsigned char *send_ptr;			// 指向发送数据的buffer
    unsigned char piece_buf[PIECE_FIX_SIZE+HEAD_SIZE+1];	//单帧的buffer
