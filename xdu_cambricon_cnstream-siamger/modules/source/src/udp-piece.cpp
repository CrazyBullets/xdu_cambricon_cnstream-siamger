 ...
 * @return 返回分片的数量
 */
int udp_piece_cut( udp_piece_t *udp_piece, unsigned char *buf, int size )
{
	if (!udp_piece || size < 0)
		return 0;
...
uint8_t *udp_piece_get( udp_piece_t *udp_piece, int index, int *got_piece_size )
{
	...
	*got_piece_size = 0;
	...
	// 所有分片数据的大小
	udp_piece->piece_buf[HEAD_POS_TOTAL_SIZE] = udp_piece->total_size >> 24;
	udp_piece->piece_buf[HEAD_POS_TOTAL_SIZE + 1] = udp_piece->total_size >> 16;
	udp_piece->piece_buf[HEAD_POS_TOTAL_SIZE + 2] = udp_piece->total_size >> 8;
	udp_piece->piece_buf[HEAD_POS_TOTAL_SIZE + 3] = (udp_piece->total_size & 0xff);
 ...
 */
int udp_piece_merge( udp_piece_t *udp_piece, unsigned char *buf, int size )
{
	unsigned char *piece_buf = NULL;
	int temp_size = 0;
	...
	// 检测头部是否有同步字
	// LOGI(UDP_PIECE_MERGE) << "check head";
	piece_buf = buf;
	temp_size = size;
	for(int i = 0; i < size; i++)
	{
		if((piece_buf[HEAD_POS_SYNC_WORD] == 0xAF) && (piece_buf[HEAD_POS_SYNC_WORD+1] == 0xAE))
			break;
		piece_buf++;
		temp_size--;
	}
	// LOGI(UDP_PIECE_MERGE) << "temp_size == " << temp_size;

	// 如果检测到同步字，且剩余数据长度还超过 分片帧头长度
	// LOGI(UDP_PIECE_MERGE) << "try while1";
	while (temp_size > HEAD_SIZE)
	{
		// 当前分片的数据长度
		int data_len = (piece_buf[HEAD_POS_P_LENGTH] << 8) + (piece_buf[HEAD_POS_P_LENGTH+1]) ;
		// LOGI(UDP_PIECE_MERGE) << "data_len = " << data_len;
		if(temp_size >= (HEAD_SIZE + data_len))
		{
			// 获取分片编号
			p_index = (buf[HEAD_POS_P_INDEX] << 8) + (buf[HEAD_POS_P_INDEX+1]);
			// LOGI(UDP_PIECE_MERGE) << "p_index = " << p_index;
			// LOGI(UDP_PIECE_MERGE) << "get udp_piece->total_size = " << udp_piece->total_size;
			if(udp_piece->total_size == 0)		// 重置后第一次收到数据
			{
				// 获取分片数据的总大小
				udp_piece->total_size = (piece_buf[HEAD_POS_TOTAL_SIZE] << 24) + (piece_buf[HEAD_POS_TOTAL_SIZE + 1] << 16) + (piece_buf[HEAD_POS_TOTAL_SIZE + 2] << 8) + (piece_buf[HEAD_POS_TOTAL_SIZE + 3]);
				// LOGI(UDP_PIECE_MERGE) << "set udp_piece->total_size = " << udp_piece->total_size;
				// 获取总的分片数量
				udp_piece->total_pieces = (piece_buf[HEAD_POS_TOTAL_PIECES] << 8) + (piece_buf[HEAD_POS_TOTAL_PIECES+1]);
				// LOGI(UDP_PIECE_MERGE) << "set udp_piece->total_pieces = " << udp_piece->total_pieces;
				udp_piece->recv_len = 0;
				udp_piece->recv_pieces  = 0;
				if(udp_piece->recv_buf)
				{
					// LOGI(UDP_PIECE_MERGE) << "try free1 ";
					free(udp_piece->recv_buf);
				}
				udp_piece->recv_buf = reinterpret_cast<unsigned char *>(malloc(udp_piece->total_size + 1));
				if ( !udp_piece->recv_buf )
				{
					// LOGW(UDP_PIECE_MERGE) << "malloc recv_buf filed";
					return(-1);
				}
			}
			...
			temp_total_size = (piece_buf[HEAD_POS_TOTAL_SIZE] << 24) + (piece_buf[HEAD_POS_TOTAL_SIZE + 1] << 16) + (piece_buf[HEAD_POS_TOTAL_SIZE + 2] << 8) + (piece_buf[HEAD_POS_TOTAL_SIZE + 3]);
			...
			if((temp_total_size != udp_piece->total_size) || (temp_total_pieces != udp_piece->total_pieces))
			{
				...
				if(udp_piece->recv_buf)
				{
					// LOGI(UDP_PIECE_MERGE) << "try free2 ";
					free(udp_piece->recv_buf);
				}
				udp_piece->recv_buf = reinterpret_cast<unsigned char *>(malloc(udp_piece->total_size + 1));
				if ( !udp_piece->recv_buf )
			...
			piece_buf += HEAD_SIZE;
			temp_size -= HEAD_SIZE;
			// LOGI(UDP_PIECE_MERGE) << "try memcpy ";
 ...
 * @return  返回-1则重组失败，返回0则正在重组中，返回1则重组成功
 */
int udp_piece_merge_ex( udp_piece_t *udp_piece, unsigned char *buf, int size )
{
	...
	// 先将接收到的数据buf拷贝到环形缓存中
	LOGI(UDP_PIECE_MERGE_EX) << "circular_buffer_write\n";
	 ...
	 * 检测头部
	 */
	LOGI(UDP_PIECE_MERGE_EX) << "try enter while1\n";
	...
	/* 如果剩余的数据长度仍大于于 */
	LOGI(UDP_PIECE_MERGE_EX) << "try enter while2\n";
	while ( circular_buffer_size( udp_piece->circular_buffer ) > HEAD_SIZE )
	{
		...
		if ( circular_buffer_size( udp_piece->circular_buffer ) >= static_cast<int>(HEAD_SIZE + data_len) )
		{
			...
			p_index = static_cast<int>((value0 << 8) + value1);
			if ( udp_piece->total_size == 0 )                                                               /* 重置后第一次收到分片 */
			{
				...
				udp_piece->total_size = static_cast<int>((value0 << 8) + value1);
				LOGI(UDP_PIECE_MERGE_EX) << "1:total_size = " << udp_piece->total_size;
				/* 计算当前分片所属分片组的总分片数量 */
				...
				udp_piece->total_pieces = static_cast<int>((value0 << 8) + value1);
			...
			circular_buffer_get( udp_piece->circular_buffer, HEAD_POS_TOTAL_SIZE, &value0 );        /* 通过索引获取当前值 */
			circular_buffer_get( udp_piece->circular_buffer, HEAD_POS_TOTAL_SIZE + 1, &value1 );
			tmp_total_size = static_cast<int>((value0 << 8) + value1);

			/* 计算当前分片所属分片组的总分片数量 */
			circular_buffer_get( udp_piece->circular_buffer, HEAD_POS_TOTAL_PIECES, &value0 );      /* 通过索引获取当前值 */
			circular_buffer_get( udp_piece->circular_buffer, HEAD_POS_TOTAL_PIECES + 1, &value1 );
			tmp_total_pieces = static_cast<int>((value0 << 8) + value1);
			udp_piece->recv_pieces++;
			if ( (tmp_total_pieces != udp_piece->total_pieces) || (tmp_total_size != udp_piece->total_size) )
			{
				...
				LOGI(UDP_PIECE_MERGE_EX) << "2:total_size = " << udp_piece->total_size;
				udp_piece->total_pieces = tmp_total_pieces;
				udp_piece->recv_pieces	= 1;
				udp_piece->recv_len	= 0;
				if ( udp_piece->recv_buf )
				{
					LOGI(UDP_PIECE_MERGE_EX) << "free2\n";
					free( udp_piece->recv_buf );
				}
				udp_piece->recv_buf = reinterpret_cast<unsigned char *>(malloc( udp_piece->total_size + 1 ));
				if ( !udp_piece->recv_buf )
			...
			udp_piece->recv_len += static_cast<int>(data_len);
			UDP_DEBUG( "recv_len = %d\n", udp_piece->recv_len );
			if ( udp_piece->recv_pieces == udp_piece->total_pieces )
			{
				udp_piece->total_pieces = 0;
				udp_piece->recv_pieces	= 0;
