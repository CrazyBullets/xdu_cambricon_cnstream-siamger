...
circular_buffer_t *circular_buffer_init( int capacity )
{
	...
	circular_buffer->data_ = reinterpret_cast<unsigned char *>(malloc( capacity ));
...
int circular_buffer_write( circular_buffer_t *circular_buffer, const unsigned char *data, int bytes )
{
	if ( bytes == 0 )
		return(0);
...
int circular_buffer_read( circular_buffer_t *circular_buffer, unsigned char *data, int bytes )
{
	if ( bytes == 0 )
		return(0);
...
/* 通过索引取值 */
int circular_buffer_get( circular_buffer_t *circular_buffer, int index, unsigned char *value )
{
