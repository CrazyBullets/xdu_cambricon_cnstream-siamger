...
edk::Matrix mat2edk(const Mat& in){
    edk::Matrix out(in.rows, in.cols);
    if(in.type() == CV_32F){
        for(int i = 0; i < in.rows; i++){
            const float* ptr = in.ptr<float>(i);
        ...
        LOGE(MAT2EDK)<<" Unsupport Mat type :"<<in.type()<<"to edk::Matrix";
        return out; 
    }
...
void kron(InputArray src1, InputArray src2,OutputArray dst){
    ...
    Mat m1 = src1.getMat();
    Mat m2 = src2.getMat();
    Mat k = Mat(m1.rows*m2.rows, m1.cols*m2.cols, m1.type());

    if(m1.type() != CV_32F){
        LOGE(KRON)<<"func kron unsupport Mat :"<<m1.type();
        return ;
    }
    ...
    InputArray inputMask)
{
    ...
    return _findTransformECC(templateImage, inputImage, warpMatrix, motionType, criteria, inputMask, 5);
}
